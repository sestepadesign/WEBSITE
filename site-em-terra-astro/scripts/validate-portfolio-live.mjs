/**
 * Validate all 48 portfolio pages on live site.
...
async function validatePage(project, lang) {
  const url = `${BASE}/${lang}/portfolio/${project.slug}/`;
  const issues = [];
  let status = 0;
  let html = '';
...
async function main() {
  ...
  for (const lang of LANGS) {
    for (const project of projects) {
      const r = await validatePage(project, lang);
