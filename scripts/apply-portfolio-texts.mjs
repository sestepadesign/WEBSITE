/**
 * Apply official WordPress portfolio texts to projects.ts (EN, ES, DE)
...
function pickParagraphs(paragraphs, max = 4) {
  const skip =
    /^(BINISSALEM|BUNYOLA|CAMPOS|CRESTATX|SANT LLORENÇ|VIDEO YOUTUBE|Strategic Landscape|Terrace Garden Design:|Santa Ponsa:|Binissalem:|Finca Garden$|General Concept$|The Project$|Spatial Organization$|Central Focus|Das Projekt$|2D Y 3D$|Finca Jardín$)/i;
...
async function main() {
  ...
  for (const slug of Object.keys(raw)) {
    ...
    if (updated) {
      ...
      console.log(`- ${slug} (no WP text)`);
    }
  }

  await fs.writeFile(PROJECTS_PATH, projectsTs, 'utf8');
  console.log('\nUpdated projects.ts descriptions from WordPress.');
}
